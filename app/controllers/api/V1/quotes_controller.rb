      ...
      def index
        @lists = List.includes([:seller, :token, :fiat_currency, payment_method: [:user, bank: [:fiat_currency]]])
                     ...
                     .where(chain_id: params[:chain_id], token: { address: params[:token_address] },
                            type: 'SellList', fiat_currency: { code: params[:fiat_currency_code] })

        @lists = @lists.sort_by(&:price)
        @list = @lists.first

        if @list
          render json: { price: @list.price, token_amount: params[:token_amount], fiat_amount: params[:fiat_amount] } , status: :ok
        else
