module Api
  module V1
    class QuickBuyController < BaseController
      def index
        total_amount_condition = "total_available_amount >= #{params[:token_amount]}" if params[:token_amount].present?
        total_fiat_condition = if params[:fiat_amount].present?
          ...
          "total_available_amount >= #{amount} / (CASE WHEN margin_type = #{List.margin_types['fixed']} THEN margin ELSE (#{token_price} + (#{token_price} * margin / 100)) END)
             AND (limit_min >= #{amount} OR limit_min IS NULL) AND (limit_max <= #{amount} OR limit_max IS NULL)"
        end
